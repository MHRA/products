resource "azuread_application" "pars-upload" {
  ...
  reply_urls              = concat(["https://${azurerm_cdn_endpoint.pars.host_name}", "https://${azurerm_storage_account.pars.primary_web_host}"], var.add_local_pars_reply_url ? ["http://localhost:3000"] : [])
