...
impl Report {
    ...
    pub(crate) fn add_skipped_duplicate(&mut self, file_name: &str, hash: &str) {
        ...
        if self.verbosity >= 1 {
            println!("Skipping {} with sha1 hash '{}' as a duplicate of an already-uploaded file.", file_name, hash);
        }
    }
    ...
    pub(crate) fn add_skipped_incomplete(&mut self, file_path: &str) {
        ...
        if self.verbosity >= 1 {
            println!("Skipping file at '{}' because it has no metadata", file_path);
        }
    }
    ...
    pub(crate) fn add_skipped_unreleased(&mut self, file_name: &str, release_state: &str) {
        ...
        if self.verbosity >= 1 {
            println!("Skipping {} with release state '{}' as it is not released to the public.", file_name, release_state);
        }
    }
